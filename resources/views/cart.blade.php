...
<div class="w-[90%] mx-auto my-10 flex flex-col lg:flex-row">
        ...
        @if(isset($cart['products']) && count($cart['products']) > 0)
            @foreach($cart['products'] as $product)
                <div class="flex flex-col md:flex-row items-center bg-gray-100 dark:bg-gray-700 rounded-lg p-4">
                    <img src="{{ asset($product['image_url']) }}" alt="{{ $product['title'] }}" class="w-full md:w-1/4 h-32 object-cover rounded-md">
                    <div class="flex-1 md:ml-4">
                        <h3 class="text-xl font-bold text-gray-800 dark:text-gray-200">{{ $product['title'] }}</h3>
                        <p class="text-gray-600 dark:text-gray-400">{{ $product['description'] }}</p>
                        <div class="flex items-center mt-2">
                            <span class="text-teal-600 font-bold text-lg">{{ number_format($product['price'], 2) }} TL</span>
            ...
            @if(isset($cart['coupon']))
                <p class="text-green-500 mt-2 md:mt-0">Coupon "{{ $cart['coupon']['coupon'] }}" has been applied!</p>
            @endif

            <!-- Coupon and Proceed to Checkout -->
            <div class="flex flex-col md:flex-row justify-between items-center mt-4">
                <!-- Add Coupon Form -->
                <form action="{{ route('cart.applyCoupon') }}" method="POST" class="flex items-center space-x-2">
                    @csrf
    ...
    <div class="w-full lg:w-1/5 bg-white dark:bg-gray-800 rounded-lg p-6 flex flex-col mt-6 lg:mt-0 lg:ml-4">
        ...
        <div class="mb-4 p-2 flex flex-col border-2 rounded-lg space-y-3 items-center">
            <img src="{{ $user['photo'] ? asset('storage/' . $user['photo']) : asset('bg/default-user.png') }}" alt="User Image" class="w-32 h-32 object-cover rounded-full">
        ...
        @if($address)
            <div class="p-2 flex flex-col border-2 rounded-lg space-y-3">
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400"><b>Street:</b> {{ $address['street'] }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400"><b>City:</b> {{ $address['city'] }}</p>
                <p class="text-sm text-gray-600 dark:text-gray-400"><b>Country:</b> {{ $address['country'] }}</p>
                <a href="{{ route('profile') }}" class="text-teal-500 hover:text-teal-700 mt-2">Edit Address</a>
            </div>
        @else
            <div class="p-2 flex flex-col border-2 rounded-lg space-y-3">
                <p class="text-sm text-gray-600 dark:text-gray-400">No delivery address found.</p>
                <a href="{{ route('profile') }}" class="text-teal-500 hover:text-teal-700 mt-2">Add Address</a>
            </div>
        @endif
    </div>
</div>
        ...
        <div id="checkoutForm">
            <div class="p-6">
                ...
                <div class="mt-4 space-y-2">
                    ...
                    @foreach($cart['products'] as $product)
                        <div class="flex justify-between">
                            ...
                            <span class="text-gray-700 dark:text-gray-300">{{ number_format($product['price'], 2) }} TL</span>
                        </div>
                    @endforeach
                    <hr class="my-2">
                    <div class="flex justify-between">
                        <span class="text-gray-700 dark:text-gray-300">Original Price:</span>
                        <span class="text-gray-700 dark:text-gray-300">{{ number_format($originalPrice, 2) }} TL</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between font-bold">
                        ...
                        <span class="text-gray-800 dark:text-gray-200">{{ number_format($finalPrice, 2) }} TL</span>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    ...
                    <p class="text-sm text-gray-700 dark:text-gray-300"><b>Name:</b> {{ $user['name'] }} {{ $user['surname'] }}</p>
                    @if($address)
                        <p class="text-sm text-gray-700 dark:text-gray-300"><b>Address:</b> {{ $address['street'] }}, {{ $address['city'] }}, {{ $address['country'] }}</p>
                    @else
